...
`wrangler.jsonc`의 `vars`에서 모델(`OPENROUTER_MODEL`), 허브 주소(`HUB_BASE_URL`), 하루 카드 수(`DAILY_CARDS`), 기준 시간대(`TIMEZONE`, 기본 `America/Denver`), 수집 기준 시각(`COLLECT_HOUR`)과 간격(`COLLECT_INTERVAL_HOURS`, 기본 4시간)을 바꿀 수 있습니다.
