...
import { DAY, getState, kstDay, kstParts, kstWeekStart, log, setState } from "./util.js";
...
export async function tick(env, { forceCollect = false } = {}) {
  ...
  const collectHour = Number(env.COLLECT_HOUR_KST ?? 6);
  ...
  const steps = [
    ...
    async () => {
      ...
      const week = weekday === 0 && hour >= 18 ? kstWeekStart(now) : kstWeekStart(now - 7 * DAY);
