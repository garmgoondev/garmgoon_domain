...
export async function buildWeeklyReport(env, weekStart) {
  ...
  const [{ results: items }, { results: videos }] = await Promise.all([
    env.DB.prepare("SELECT headline, kind, signal, category, tags, source_label FROM items WHERE status = 'published' AND day BETWEEN ? AND ? ORDER BY score DESC LIMIT 200")
      .bind(start, end)
      .all(),
    env.DB.prepare(
